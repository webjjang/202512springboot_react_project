...
function PacsList(){
  ...
  useEffect(
    function(){
      console.log("PacsList :: useEffect 실행해서 데이터 가져오기 시작")
      // react(:5173) -> spring boot(:80-http인 경우 생략)로 데이터 요청
      api.get("http://localhost/pacs/list.do?" + query)
      .then((response) => {
    ...
    (vo) => {
      return (
        <tr className="dataRow" key={vo.no}
         onClick={() => navigate(`/pacs/view?no=${vo.no}&inc=1`)}>
          <td className="no" >{vo.no}</td>
          <td>{vo.title}</td>
          <td>{vo.id}</td>
          <td>{vo.name}</td>
          {/* spring boot 서버에서 이미지 데이터를 가져와서 표시해준다. img 태그 사용 */}
          <td><img src={`http://localhost/upload/pacs/${vo.fileName}`} alt={vo.title} style={{ maxWidth: '50px', maxHeight: '50px' }} /></td>
  ...
  return(
    <>
      ...
      <Link to={"/pacs/write"} className="btn btn-primary">등록</Link>
